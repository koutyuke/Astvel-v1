...
const apiVoiceChannel = async (req: Request, res: Response, client: Client<boolean>) => {
  ...
  try {
    ...
    const channel = guild?.channels.cache.find(c => c.id === channelId && c.type === ChannelType.GuildVoice) as
      | VoiceChannel
      | undefined;
    const everyone = guild?.roles.cache.find(r => r.id === guildId)?.permissions.bitfield;

    if (guild === undefined || user === undefined || channel === undefined || everyone === undefined) {
      res.status(400).json({ message: "Bad Request" });
      return;
    }
...
export default apiVoiceChannel;
